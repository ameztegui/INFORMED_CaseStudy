...
knitr::opts_knit$set(root.dir = "D:/Aitor/INFORMED_CaseStudy")
...
```{r mapa_solsones, eval = FALSE, fig.cap="Figure 1. (a) Location of the Solsones county within Catalonia, and (b) location of the 261 study plots classified according to the dominant species",fig.width = 9,fig.align="center"}

mdt <- raster("./OtherInputs/SOL_MDT.tif")
...
timber_species %>% 
    ggplot(aes(Year,zoo::rollmean(TotalV_Alive_mean,45,na.pad=T), color = Management)) +
...
selected_vars <- c("TotalV_Harvested", "C_Inc", "Mushroom", "Temp", "Prec")
...
es_solsones_period_bau <- es_plot_period  %>%
  ...
  mutate(Hv =  TotalV_Harvested_sum_sum / TotalV_Harvested_sum_sum[Management == "BAU"],
         ...
         NULL) %>%
  gather(ES, value, Hv:Ms) %>%
  mutate(ES= factor(ES, levels = c("Hv", "Cb", "Ms")))
...
new_selected_vars <- c("TotalV_Harvested_sum", "C_Inc_sum", "Mushroom_sum", "Prec_sum")
new_names = c("Harvest", "Carbon", "Mushroom",  "Precipitation")

BAU_45 <- es_plot_all %>%
  filter(RCP == "45", Management == "BAU") %>% ungroup() %>%
    ggpairs( mapping = aes(color = Forest_Type, alpha=0.6), columns =new_selected_vars,columnLabels = new_names,
           lower=list(continuous="smooth")) +
  ggtitle("Business as usual | Moderate climate change") +
...
BAU_85 <- es_plot_all %>%
  filter(RCP == "85", Management == "BAU") %>% ungroup() %>%
    ggpairs( mapping = aes(color = Forest_Type, alpha=0.6), columns =new_selected_vars,columnLabels = new_names,
           lower=list(continuous="smooth")) +
    ggtitle("Business as usual | Severe climate change") +
  theme_bw() + theme(strip.background =  element_blank())
...
BIO_45 <- es_plot_all %>%
  filter(RCP == "45", Management == "BIO") %>% ungroup() %>%
    ggpairs( mapping = aes(color = Forest_Type, alpha=0.6), columns =new_selected_vars,columnLabels = new_names,
           lower=list(continuous="smooth")) +
     ggtitle("Biomass production | Moderate climate change") +
 theme_bw() + theme(strip.background =  element_blank())
...
BIO_85 <- es_plot_all %>%
  filter(RCP == "85", Management == "BIO") %>% ungroup() %>%
    ggpairs( mapping = aes(color = Forest_Type, alpha=0.6), columns =new_selected_vars,columnLabels = new_names,
           lower=list(continuous="smooth")) +
    ggtitle("Biomass production | Severe climate change") +
theme_bw() + theme(strip.background =  element_blank())
...
CAR_45 <- es_plot_all %>%
  filter(RCP == "45", Management == "CAR") %>% ungroup() %>%
    ggpairs( mapping = aes(color = Forest_Type, alpha=0.6), columns =new_selected_vars,columnLabels = new_names,
           lower=list(continuous="smooth")) +
      ggtitle("Carbon storage | Moderate climate change") +
theme_bw() + theme(strip.background =  element_blank())
...
CAR_85 <- es_plot_all %>%
  filter(RCP == "85", Management == "CAR") %>% ungroup() %>%
    ggpairs( mapping = aes(color = Forest_Type, alpha=0.6), columns =new_selected_vars,columnLabels = new_names,
           lower=list(continuous="smooth")) +
      ggtitle("Carbon storage | Severe climate change") +
theme_bw() + theme(strip.background =  element_blank())
...
ADA_45 <- es_plot_all %>%
  filter(RCP == "45", Management == "ADA") %>% ungroup() %>%
    ggpairs( mapping = aes(color = Forest_Type, alpha=0.6), columns =new_selected_vars,columnLabels = new_names,
           lower=list(continuous="smooth")) +
          ggtitle("Adaptation | Moderate climate change") +
theme_bw() + theme(strip.background =  element_blank())
...
ADA_85 <- es_plot_all %>%
  filter(RCP == "85", Management == "ADA") %>% ungroup() %>%
    ggpairs( mapping = aes(color = Forest_Type, alpha=0.6), columns =new_selected_vars,columnLabels = new_names,
           lower=list(continuous="smooth")) +
       ggtitle("Adaptation | Severe climate change") +
theme_bw() + theme(strip.background =  element_blank())
...
# new_selected_vars <- c("Volume_Harvest_sum", "Carbon_Inc_sum", "Mushroom_sum", "BW_sum", "ER_sum", "Prec_sum")
# new_names = c("Harvest", "Carbon", "Mushroom", "Blue water", "Erosion", "Precipitation")

BAU_45_time <- es_year_all %>%
  filter(RCP == "45", Management == "BAU") %>% ungroup() %>%
    ggpairs( mapping = aes(color = Forest_Type), columns =new_selected_vars,columnLabels = new_names,
           lower=list(continuous="smooth")) +
          ggtitle("Business as usual | Moderate climate change") +
theme_bw() + theme(strip.background =  element_blank()) 
